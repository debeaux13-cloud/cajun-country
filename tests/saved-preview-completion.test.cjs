...
test('ready PATCH persists completion time once and reopening does not move expiry',async()=>{
  const source=fs.readFileSync(path.join(__dirname,'..','pages','api','internal','preview-pipeline','[id].js'),'utf8')
    ...
    .replace("import{persistSavedPreview}from'../../../../lib/saved-previews';","const {persistSavedPreview}=saved;").replace("import{normalizeMoods}from '../../../../lib/mcs-contract';","const {normalizeMoods}=contract;")
  ...
  const context={module:{exports:{}},process:{env:{MCS_WORKER_SECRET:'secret',BLOB_READ_WRITE_TOKEN:'blob'}},Date:FixedDate,Number,JSON,Buffer,console:{log:()=>{}},blob:{put:async(pathname,value)=>writes.push({pathname,value:JSON.parse(value)}),head:async()=>({})},subject:{subjectContract:()=>({})},saved:{persistSavedPreview:async(...args)=>calls.push(args)},contract:{normalizeMoods:()=>['surprise me']}};
