...
function load(records){
  const source=fs.readFileSync(path.join(__dirname,'..','lib','saved-previews.js'),'utf8')
    ...
    .replace(/export async function /g,'async function ').replace(/export function /g,'function ').replace(/export \{DAY,active,moviePath,publicRecord\};/, 'module.exports={DAY,active,moviePath,publicRecord,ensureSavedPreview,getSavedPreview,getSavedPreviewByShare,storedMovieAvailable};');
...
test('retains a stored movie for 24 hours and expires only after expiresAt',async()=>{const records={};const api=load(records);const start=Date.parse('2026-08-28T00:00:00.000Z');const record=await api.ensureSavedPreview(ID,'token',start);assert.equal(record.expiresAt,'2026-08-29T00:00:00.000Z');assert.ok(api.publicRecord(record,Date.parse(record.expiresAt)));assert.equal(api.publicRecord(record,Date.parse(record.expiresAt)+1),null);});
