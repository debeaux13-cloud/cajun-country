...
function pathname(id,kind,scene){const suffix=scene?'-'+String(scene):'';return 'mcs/jobs/'+String(id)+'/'+String(kind||'preview')+suffix+'.bin'}
export default async function handler(req,res){
  if(!auth(req))return res.status(401).send('Unauthorized');
  const{id,kind='preview'}=req.query;const scene=String(req.query.scene||req.headers['x-mcs-scene']||'').trim();const path=pathname(id,kind,scene);const token=process.env.BLOB_READ_WRITE_TOKEN;
