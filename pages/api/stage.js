...
async function makePlan(idea,moods){
  ...
  const r=await fetch('https://ai-gateway.vercel.sh/v1/chat/completions',{
    ...
    body:JSON.stringify({model:'openai/gpt-4o-mini',messages:[
      {role:'system',content:'You are Stage for Main Character Studios by Tiffani. Write exactly 30 numbered cinematic moving scenes for a 5-minute personalized animated movie, about 10 seconds per scene. Scenes 1-6 form a compelling 60-second preview with a hook and emotional payoff that naturally continues into the paid movie. Every scene must contain visible character action, position change, environmental interaction, and camera movement. Keep character identity consistent. Return only the numbered plan.'},
