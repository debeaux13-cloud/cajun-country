...
export const workerConfigurationPresent = () => Boolean(previewWorkerEnvironment() && process.env.MCS_WORKER_SECRET && (process.env.Run_Pod_Key || process.env.RUNPOD_API_KEY) && process.env.BLOB_READ_WRITE_TOKEN && process.env.VERCEL_URL);
...
export async function submitPreviewJob({ id, idempotencyKey }) {
  if (!previewWorkerEnvironment()) throw new Error('Preview worker is unavailable outside Preview');
