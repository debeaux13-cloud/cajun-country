...
const CLAIM_INDEX_PREFIX='mcs/preview-claim-index';
...
function claimIndexPath(mcsJobId){return`${CLAIM_INDEX_PREFIX}/${mcsJobId}.json`}
...
async function writeClaimIndex(id,claim,token){
  const mcsJobId=String(claim?.mcsJobId||'').trim();
  if(!PREVIEW_ID.test(mcsJobId))return;
  await put(claimIndexPath(mcsJobId),JSON.stringify({
    requestId:id,mcsJobId,pathname:claimPath(id),updatedAt:claim.updatedAt||new Date().toISOString()
  }),{access:'private',addRandomSuffix:false,allowOverwrite:true,contentType:'application/json',token});
}

export async function getPreviewClaimByMcsJobId(mcsJobId,token){
  const id=String(mcsJobId||'').trim();
  if(!PREVIEW_ID.test(id))throw new Error('A valid preview job is required');
  const index=await privateJson(claimIndexPath(id),token);
  if(index?.pathname){
    const indexedClaim=await privateJson(String(index.pathname),token);
    if(String(indexedClaim?.mcsJobId||'')===id)return indexedClaim;
  }
  let cursor;
  let newest=null;
  do{
    const result=await list({prefix:`${CLAIM_PREFIX}/`,limit:1000,cursor,token});
    for(const blob of result.blobs||[]){
      const claim=await privateJson(blob.pathname,token);
      if(String(claim?.mcsJobId||'')!==id)continue;
      if(!newest||new Date(claim?.updatedAt||claim?.createdAt||blob.uploadedAt||0)>new Date(newest.claim?.updatedAt||newest.claim?.createdAt||newest.blob?.uploadedAt||0))newest={claim,blob};
    }
    cursor=result.cursor||result.nextCursor||null;
  }while(cursor);
  if(newest)await writeClaimIndex(String(newest.claim.requestId||newest.blob.pathname.split('/').pop().replace(/\.json$/,'')),newest.claim,token);
  return newest?.claim||null;
}
...
async function writeClaim(id,claim,token,{overwrite}){
  ...
  await writeClaimIndex(id,claim,token);
  return claim;
}
