...
export function classifyPreviewClaim(claim,requestHash){
  ...
  if(claim?.status==='retrying')return{state:'reserved',claim};
...
export async function retryFailedPreviewClaim({id,claim,requestHash,mcsJobId,token}){
  if(String(process.env.VERCEL_ENV||'')!=='preview')throw new Error('Preview retry is unavailable outside Preview');
  if(Number(claim?.retryCount||0)>=1)throw new Error('This Preview render has already used its one replacement attempt.');
  const replacement={version:1,requestId:normalizePreviewRequestId(id),requestHash,mcsJobId,status:'submitting',retryCount:Number(claim?.retryCount||0)+1,previousJobId:String(claim?.jobId||''),previousMcsJobId:String(claim?.mcsJobId||''),previousRequestHash:String(claim?.requestHash||''),createdAt:claim?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};
  await writeClaim(replacement.requestId,replacement,token,{overwrite:true});
  return{state:'reserved',claim:replacement};
}
